...
**Правило плана перед стартом.** После того как пользователь подтвердил начало работы, агент не переходит сразу к реализации. Сначала он составляет план выполнения задачи (что и в каком порядке будет делать) и отдаёт его пользователю на рецензию. Только после того как пользователь одобрил план (или внёс правки и одобрил скорректированный) — начинается реализация.

**Статус "в работе".** Как только план одобрен и агент реально приступает к реализации — статус сразу меняется на "в работе" (план составляется/утверждается ещё в "к выполнению", в "в работе" задача переходит только когда началась настоящая работа).
