...
1. **Взять.** Issue с лейблом `qa` в статусе "к выполнению" → поставить "в работе". Статусы ведутся в реальном времени: идентификаторы доски и команды — в `.claude/board.md` умбрелла-репозитория.
