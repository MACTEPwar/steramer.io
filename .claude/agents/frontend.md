...
1. **Взять.** Issue с лейблом `frontend` в статусе "к выполнению" или "возвращена в работу" → поставить "в работе". Статусы ведутся в реальном времени: идентификаторы доски и команды — в `.claude/board.md` умбрелла-репозитория.
