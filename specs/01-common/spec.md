...
*Состояние: адаптив в значимом объёме реализован только для страницы «Новости». У шапки (`ШАП`) — пока только центрирование на очень широких экранах; компактная компоновка описана этим блоком (`ШАП-Ф-02` — `ШАП-Ф-14`), но не реализована. Остальные страницы свёрстаны под широкий экран и требованиям этого блока пока не удовлетворяют — это известное расхождение, а не отдельное исключение для каждой страницы.* ⚠
...
*Общий каркас страниц — единственный блок, присутствующий буквально везде, поэтому его теснота на узком экране бьёт по всему сайту разом, а не по одной странице.*

#### Общие

- **ШАП-О-01** — Шапка присутствует на всех страницах и содержит логотип (ссылка на главную), основное меню, кнопку поддержки и область входа: кнопку «Войти» для гостя либо аватар/имя со ссылкой на профиль для вошедшего, с дополнительной ссылкой на панель управления для роли администратора (`АВТ-О-05`).
- **ШАП-О-02** — Текущий раздел в меню визуально выделен — на любой компоновке, включая свёрнутое меню (`ШАП-Ф-03`).
- **ШАП-О-03** — На компактной компоновке меню, кнопка поддержки и область входа не пропадают и не теряют ни одного пункта (`АДП-О-08`) — они переходят в открываемое меню (`ШАП-Ф-02`) вместо того, чтобы стоять в строке шапки.
...
*Строка из пяти пунктов меню, кнопки поддержки и области входа не помещается на компактной ширине без переноса или горизонтальной прокрутки — то и другое запрещено (`АДП-Ф-10`). Решение — то же, что уже работает в проекте для `NewsDetailModal` (`ModalHost`, sheet-вариант по `SMALL_QUERY`): выезжающая панель вместо строки, без нового механизма.*

- **ШАП-Ф-02** — На компактной компоновке строка навигации заменяется переключателем (иконка «гамбургер»/«крестик» по состоянию) — одним и тем же элементом, открывающим и закрывающим меню. Логотип и переключатель остаются в строке шапки всегда, независимо от того, открыто меню или нет.
- **ШАП-Ф-03** — Открытое меню содержит то же и в том же порядке, что строка шапки на широкой компоновке: пункты навигации (с выделением текущего, `ШАП-О-02`), кнопку поддержки, область входа/профиля и, при роли администратора, ссылку на панель управления.
- **ШАП-Ф-04** — Меню закрывается: повторным нажатием того же переключателя (`ШАП-Ф-02` — один элемент на оба действия, второй, отдельной кнопки закрытия внутри панели нет), кликом по затемнённой области под панелью (`ШАП-Ф-14`) и клавишей отмены (`ДСТ-Ф-01`).
- **ШАП-Ф-05** — Клик по любому пункту меню — включая ссылку на раздел, в котором человек уже находится, — закрывает меню. Реальный переход не обязателен: важно, что человек явно выбрал пункт, а не то, сменился ли адрес.
- **ШАП-Ф-06** — Порог, с которого шапка переключается на компактный вид, — тот же класс компоновки, что и у всей остальной страницы (`АДП-О-04`), не отдельно подобранное число: своя граница у шапки разошлась бы с остальной страницей при первой же правке порогов (`АДП-Ф-03`).
- **ШАП-Ф-07** — Средняя и широкая компоновки используют одну и ту же несвёрнутую строку шапки — отдельного промежуточного вида для средней компоновки шапка не заводит (обоснование — в «Принятых решениях»).
- **ШАП-Ф-08** — Переключатель меню и каждый пункт внутри открытого меню — область нажатия не меньше минимальной (`АДП-Ф-14`), с зазором между соседними (`АДП-Ф-15`).
- **ШАП-Ф-09** — При открытии меню фокус клавиатуры переходит внутрь и не покидает его, пока оно открыто (`ДСТ-Ф-01`); при закрытии — возвращается на переключатель, а не теряется.
- **ШАП-Ф-10** — Пока меню открыто, содержимое страницы под ним недоступно указателю и клавиатурной табуляции — иначе фокус мог бы уйти под визуально скрытую панель незаметно для человека.
- **ШАП-Ф-11** — Если по ходу изменения размера окна компоновка перестаёт быть компактной, пока меню открыто, — меню закрывается вместе с этим переходом, а строка навигации возвращает обычный вид. Это не потеря состояния в смысле `АДП-Ф-07`: у открытого меню нет аналога в некомпактной компоновке, сохранять там нечего.
- **ШАП-Ф-12** — Пока меню открыто, страница под ним не прокручивается — прокручивается только само меню, если его содержимое не помещается по высоте.
- **ШАП-Ф-13** — Меню открывается панелью на всю ширину экрана, встающей ПОД строкой шапки (не боковой выезжающей панелью и не перекрытием во весь экран поверх самой шапки). Содержимого мало — пять пунктов раздела, кнопка поддержки, область входа, при необходимости ссылка на панель управления, — оно не требует более тяжёлого паттерна, а этот держит логотип и переключатель (`ШАП-Ф-02`) на виду и на прежнем месте всё время, пока меню открыто.
- **ШАП-Ф-14** — Пока меню открыто, содержимое страницы под панелью притемнено — тот же приём, что затемнение фона у `ModalHost`. Не только функциональный признак «остальное сейчас недоступно» (`ШАП-Ф-10`), но и видимый: человек должен понимать это, не пробуя ткнуть мимо.
...
| `АДП` целиком | ОБЩ | Адаптив в значимом объёме реализован только для страницы «Новости»; главная, профиль и админка свёрстаны под широкий экран |
| `ШАП-Ф-02` — `ШАП-Ф-14` | ОБЩ | Компактная компоновка шапки не реализована: строка навигации не сворачивается в меню при нехватке ширины, ни горизонтальной прокрутки, ни переноса при этом не предотвращено (`АДП-Ф-10`) — риск нарушения растёт с любой правкой состава шапки |
| `АДП-О-10` | ОБЩ | Поведение блоков по компоновкам описано только в [НОВ](../05-news/spec.md) (`РАЗ`) и [ШАП](#шап--шапка-и-навигация) выше; у остальных страниц такого описания нет |
...
- **Макеты существуют только под широкий экран.** Компактная раскладка спроектирована без референса, поэтому её отступы и размеры — предмет визуальной приёмки, а не сверки с эталоном. Компактная шапка (`ШАП`) спроектирована тем же способом и по той же причине: макета мобильного меню нет ни в одном Figma-экспорте.
- **Компактная шапка — выезжающее меню по образцу `ModalHost`, не новый механизм.** В проекте уже есть работающий и проверенный тестами приём переключения на компактный вид по ширине (`BreakpointObserver`/`SMALL_QUERY`, sheet-вариант `ModalHost`, единственный текущий потребитель — `NewsDetailModal`). Заводить для шапки отдельный способ определять «тесно» или отдельную реализацию выезжающей панели значило бы дублировать то, что уже работает, ради компонента, у которого нет содержательных отличий в этой части.
- **Кнопка поддержки и область входа остаются на виду, в меню уходит только навигация.** Рассмотрено три варианта: спрятать всё за одним переключателем, оставить всё в строке ценой переноса/прокрутки (обе запрещены — `АДП-Ф-10`), либо разделить — второстепенное (пять пунктов раздела) сворачивается, то, что человек ищет глазами в первую секунду (поддержать, войти, кто я) остаётся видимым. Выбран третий: он не прячет действие, ради которого часть аудитории и заходит на сайт, за лишним нажатием.
- **У средней компоновки нет отдельного вида шапки.** Строка из логотипа, пяти пунктов, кнопки и области входа умещается в ширину планшетного порога (768) без переноса — сжимать её ради компоновки, которая и так технически широкая, незачем. Отдельный вид завёлся бы третьим состоянием там, где хватает двух, вопреки `АДП-Ф-03`.
