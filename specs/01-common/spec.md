...
| `АДП-О-12`, `РАЗ-О-02`, `РАЗ-Ф-01` — `РАЗ-Ф-04` | ОБЩ, [НОВ](../05-news/spec.md) | Положение ленты привязано к тому же порогу, что и раскладка витрины: на планшете альбомом лента встаёт сбоку и оставляет витрине 134 точки вместо эталонных 1030 |
...
| `АДП-Ф-23`, `АДП-Б-01` | ОБЩ | Изображение отдаётся одним размером на все экраны и на все места: телефон грузит десктопный вариант, мелкая ячейка витрины — тот же файл, что крупная |
| `АДП-Ф-30` — `АДП-Ф-32`, `ЗАК-Ф-12` — `ЗАК-Ф-15` | ОБЩ, [НОВ](../05-news/spec.md) | Поведение изображения при нехватке места, при загрузке и при неудачной загрузке нигде не задано: в тесной карточке витрины изображение вырождается в полосу, битая ссылка даёт сломанный значок |
...
Отдельно: `РАБ-Ф-01` — `РАБ-Ф-03`, `РАБ-Ф-07`, `РАБ-Ф-08` ([АЗК](../02-admin/05-pinned/spec.md)) — редактор витрины выбирает раскладку через список типовых устройств вместо прямого переключателя и не даёт проверить тесный край.
